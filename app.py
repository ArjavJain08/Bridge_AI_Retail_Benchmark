...
def load_data():
    try:
        ...
        # Explicitly force a rebuild if the database is blank
        if data.empty:
            raise ValueError("Database is empty, forcing rebuild.")
        return data
        
    except Exception:
        # Build the data on the fly
        api_pipeline.run_pipeline()
        
        # Connect to the fresh data
        conn = sqlite3.connect('retail_advanced.db')
        data = pd.read_sql_query("SELECT * FROM products", conn)
        conn.close()
        return data
...
with tab4:
    ...
    c1, c2 = st.columns(2)
    c1.download_button("📥 Export as PSV", filtered_df.to_csv(sep='|', index=False).encode('utf-8'), "benchmark.psv")
    c2.download_button("📥 Export as CSV", filtered_df.to_csv(index=False).encode('utf-8'), "benchmark.csv")
